...
class EmailService:
    ...
    def send_single_email(self, student: Student, subject: str = None, body: str = None) -> bool:
        """Sends an email to a student based on priority: Real > Test."""
        if not self.gmail_user or not self.gmail_password:
            logger.error("GMAIL credentials not configured in environment.")
            return False

        # Recipient Selection Rule:
        if student.preferred_email_type == "test":
            recipient_email = student.test_email or student.email
        else:
            recipient_email = student.real_email or student.email
